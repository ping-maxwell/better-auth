...
## MySQL and ID Generation

<Callout type="warn">
  If you're using the Drizzle adapter with `provider: "mysql"` and `advanced.database.generateId` set to `false`, make sure your `drizzle-orm` version is **0.32.0 or later**. This version introduced `$returningId()`, which the adapter uses to safely retrieve inserted rows on MySQL.

  If you're using Drizzle's `$defaultFn()` on your primary key columns to generate IDs (e.g. CUID2, UUIDv4), you should either:
  - **Move the ID generation to Better Auth** by setting `advanced.database.generateId` to a custom function, or
  - **Ensure `drizzle-orm` >= 0.32.0** so `$returningId()` is available.

  See [MySQL ID Generation](/docs/adapters/mysql#id-generation-on-mysql) for more details.
</Callout>
