...
The current Drizzle adapter (imported from `better-auth/adapters/drizzle`) uses Drizzle Relations v1.
To use Drizzle Relations v2, you need to install the `@better-auth/drizzle-adapter` package:

```package-install
npm install @better-auth/drizzle-adapter
```

After installing, update your imports to use the relations-v2 adapter:
...
// your app's main relations which should use defineRelations
import { relations } from './app-schema';

export const db = drizzle({
	client,
	// authRelations uses defineRelationsPart,
	// so it must come after the main relations.
	// https://orm.drizzle.team/docs/relations-v1-v2#relations-parts
