...
* `database`: Database configuration options (⚠️ **MySQL users**: setting `generateId` to `false` with non-sequential IDs can cause the adapter to return the wrong row — see [MySQL ID Generation](/docs/adapters/mysql#id-generation-on-mysql))
